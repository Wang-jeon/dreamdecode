...
}`,r=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:l}]}],generationConfig:{temperature:.8,maxOutputTokens:512}})});if(!r.ok){const O=await r.text();throw new Error(`Gemini API ${r.status}: ${O}`)}const M=(((S=(k=(x=(v=(w=(await r.json()).candidates)==null?void 0:w[0])==null?void 0:v.content)==null?void 0:x.parts)==null?void 0:k[0])==null?void 0:S.text)??"").trim().replace(/^```(?:json)?\n?/,"").replace(/\n?```$/,"");return JSON.parse(M)}async function $(e){return"AIzaSyByPQKqIdcxj-QZkQXq_xf-1K5DqEYnxG0".length>10?N(e):Y(e)}j(document.getElementById("star-canvas"));const q=document.getElementById("app");q.innerHTML=`
...
`;const u=document.getElementById("dream-input"),b=document.getElementById("decode-btn"),z=document.getElementById("loading"),f=document.getElementById("result-card"),p=document.getElementById("error-msg"),F=document.getElementById("share-btn"),_=document.getElementById("res-symbol"),H=document.getElementById("res-archetype"),G=document.getElementById("res-emotion"),J=document.getElementById("res-insight"),Q=document.getElementById("res-keywords");let h=null;function C(e){z.classList.toggle("active",e),b.disabled=e}function g(e){p.textContent=e,p.classList.add("active")}function U(){p.textContent="",p.classList.remove("active")}function X(e){_.textContent=e.symbol,H.textContent=e.archetype,G.textContent=e.emotion,J.textContent=e.insight,Q.innerHTML=e.keywords.map(t=>`<span class="keyword-tag">${t}</span>`).join(""),f.classList.add("active"),f.scrollIntoView({behavior:"smooth",block:"nearest"})}b.addEventListener("click",async()=>{const e=u.value.trim();if(!e){g("Please describe your dream before decoding."),u.focus();return}if(e.length<10){g("Please add a bit more detail (at least a sentence)."),u.focus();return}U(),f.classList.remove("active"),C(!0);try{const t=await $(e);h=t,X(t)}catch(t){const i=t instanceof Error?t.message:String(t);console.error("[DreamDecode]",t),g(`Error: ${i}`)}finally{C(!1)}});F.addEventListener("click",()=>{if(!h)return;const e=["🔮 My dream was decoded by DreamDecode","",`Symbol: ${h.symbol}`,`Archetype: ${h.archetype}`,"",`"${h.insight.slice(0,160)}..."`,"","#DreamDecode #DreamMeaning #AI","https://wang-jeon.github.io/dreamdecode/"].join(`
